...
config['staging'] = prefs[:heroku_staging]
config['deploy']  = prefs[:heroku_deploy]
config['domain']  = prefs[:heroku_domain]

accounts_unsupported = run "heroku accounts | grep 'is not a heroku command'"

account_cmd = ""
unless accounts_unsupported
  heroku_account = ask_wizard "Which Heroku account?"
...
sendgrid = ask_wizard "Add sendgrid:starter addon?"
